...
export async function checkCredits(provider: Provider): Promise<boolean> {
  try {
    const command = provider === 'copilot' 
      ? 'copilot'
      : provider;
    
    const args = provider === 'amp'
    ...
    if (/rate.?limit|quota|too many|capacity|overloaded|try again|exceeded|insufficient|credit/i.test(output)) {
      return false;
    }
...
export async function runAgent(provider: Provider): Promise<string> {
